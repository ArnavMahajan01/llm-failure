...
**Git / Windows:** Commit experiment JSON from `results/raw/` only. The Colab folder
`llm-failure-7b-results/raw/` holds duplicate copies with doubled filenames and often
exceeds Windows path limits (`Filename too long`). Track `llm-failure-7b-results/manifest.json`,
`manifest.csv`, `logs/`, and `status/` for run metadata instead.

Before adding large result trees on Windows, enable long paths for this repo:
