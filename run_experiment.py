...
def parse_arguments():
    ...
    parser.add_argument("--resume-file", type=str, default=None, help="Resume a partial JSON result file")
...
def validateResumeResults(results: list, conditionRunning: list, total: int, resumeFile: str):
    """Ensure a resume file belongs to the same single-condition run."""
    if len(conditionRunning) != 1:
        raise ValueError("Resume is only supported for single-condition runs.")

    expectedCondition = conditionRunning[0]
    if len(results) > total:
        raise ValueError(
            f"Resume file has {len(results)} rows, but this run expects {total}: {resumeFile}"
        )

    for index, item in enumerate(results):
        conditions = item.get("conditions", {})
        if set(conditions.keys()) != {expectedCondition}:
            raise ValueError(
                f"Resume file condition mismatch at row {index}. "
                f"Expected only '{expectedCondition}', found {sorted(conditions.keys())}. "
                f"File: {resumeFile}"
            )
        ...
        if sample_id != index:
            raise ValueError(
                f"Resume file sample_id mismatch at row {index}. "
                f"Expected {index}, found {sample_id}. File: {resumeFile}"
            )
...
def runbenchmarkOnModel(model, tokenizer, modelname: str, benchmark: str, conditionRunning: list, numSamples: int, resumeFile: str = None) -> list:
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")
    outFile = resumeFile or f"{benchmarkDir}/{name}__{benchmark}__{timestamp}.json"

    allResults = []
    if resumeFile and os.path.exists(resumeFile):
        with open(resumeFile, "r", encoding="utf-8") as f:
            allResults = json.load(f)
        validateResumeResults(allResults, conditionRunning, total, resumeFile)
        print(f"  Resuming from: {resumeFile}")
        print(f"  Already completed: {len(allResults)} / {total} questions")

    if len(allResults) >= total:
        print(f"  Resume file already has {len(allResults)} questions. Nothing left to run for {benchmark}.")
        return allResults

    for i, item in enumerate(evaluationSize[len(allResults):], start=len(allResults)):
        question = item["question"]
        answer = item["answer"]
